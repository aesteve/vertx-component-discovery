...
public class MavenCentralCrawler implements Crawler {
	...
	private TaskReport<Artifact> currentReport;
	...
	@Override
	public void scan(Consumer<TaskReport<Artifact>> imDone) {
		System.out.println("Start scanning maven central");
		artifacts = new ArrayList<Artifact>();
		currentReport = new TaskReport<Artifact>("Scan maven central");
		currentReport.start();
		try {
	...
	public void fetchCurrentPage(Consumer<TaskReport<Artifact>> imDone) throws Throwable {
	...
	private void terminated() {
		currentReport.terminate(counter.getFetched());
	}

	private void failWithCause(Consumer<TaskReport<Artifact>> handler, Throwable t) {
		currentReport.fail(t);
		handler.accept(currentReport);
	}

	private void scanSuccess(Consumer<TaskReport<Artifact>> handler) {
		currentReport.terminate(counter.getFetched(), artifacts);
		System.out.println("Success ? " + currentReport.name());
		handler.accept(currentReport);
	}

	private void failWithStatus(Consumer<TaskReport<Artifact>> handler, int status) {
