...
public class MavenCentralCrawler implements Crawler {
    ...
    @Override
    public void scan(Consumer<ScanReport> imDone) {
        System.out.println("Start scanning maven central");
        artifacts = new ArrayList<Artifact>();
        currentReport = new ScanReport();
        currentReport.startTime = System.currentTimeMillis();
        try {
    ...
    public void fetchCurrentPage(Consumer<ScanReport> imDone) throws Throwable {
    ...
    private void terminated() {
        currentReport.endTime = System.currentTimeMillis();
        currentReport.itemsScanned = counter.getFetched();
    }

    private void failWithCause(Consumer<ScanReport> handler, Throwable t) {
        terminated();
        currentReport.failed = true;
        currentReport.failedCause = t;
        handler.accept(currentReport);
    }

    private void scanSuccess(Consumer<ScanReport> handler) {
        terminated();
        currentReport.failed = false;
        currentReport.result = artifacts;
        handler.accept(currentReport);
    }

    private void failWithStatus(Consumer<ScanReport> handler, int status) {
        terminated();
        currentReport.failed = true;
        currentReport.failedStatus = status;
        handler.accept(currentReport);
    }
