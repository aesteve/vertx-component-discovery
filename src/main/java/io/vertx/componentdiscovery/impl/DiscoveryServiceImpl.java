...
public class DiscoveryServiceImpl implements DiscoveryService {
    ...
    @Override
    public void crawl(Handler<AsyncResult<FullScanReport>> reports) {
        System.out.println("Start crawling");
        FullScanReport fullReport = new FullScanReport();
        fullReport.start();
        if (crawlers.isEmpty()) {
            fullReport.end();
        ...
        crawlers.forEach(crawler -> {
            crawler.scan(singleReport -> {
                fullReport.addReport(singleReport);
                if (fullReport.nbReports() == crawlers.size()) {
                    reports.handle(new DiscoveryAsyncResult(fullReport));
                }
            });
        });
